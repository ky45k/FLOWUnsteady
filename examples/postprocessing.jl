...
sims_to_plot = [ # run_name, style, color, alpha, label
                ("forward_3_6000", "-", "dodgerblue", 1.0, "rVPM - mid-high fidelity"),
                # ("kuam_mid_low", ":", "dodgerblue", 0.5, "rVPM - mid-low fidelity")
              ]
...
nrevs_to_average = 3        # Number of revolutions to average
...
for (run_name, stl, clr, alpha, lbl) in sims_to_plot
    ...
    # rotor_labels = ["Left Rotor", "Right Rotor"]
    # rotor_colors = ["dodgerblue", "orangered"]  # 원하는 색상 지정

    # for i in 1:nrotors
    #     ax.plot(simdata[2:end, 1]./360, simdata[2:end, 3 + (i-1)*4 + 1+coli], stl;
    #             alpha=alpha, label=lbl*" - "*rotor_labels[i], color=rotor_colors[i], linewidth=1.0)
    # end
...
for (run_name, _, _, _, lbl) in sims_to_plot
    ...
    global str *= "| $(lbl)$(" "^nspaces) |     $(round(CTmean[run_name], digits=5))    | $(round(100*abs(CTmean[run_name]-CTexp)/CTexp, digits=1))% |\n"
end
...
for (run_name, _, _, _, _) in sims_to_plot
    ...
    nums      = range(19, 20; length = nsteps_per_rev[run_name]+1)
...
# read_path = joinpath(sims_path, "V_stacked_180_mid_high_1_7000")

# (rs, Gamma,
#     Np, Tp) = uns.postprocess_bladeloading(read_path;
#                                             O           = zeros(3),
#                                             rotor_axis  = rotor_axis,
#                                             filename    = "V_stacked_180_mid_high_1_7000_Rotor_Blade1_vlm.2.vtk",
#                                             fieldsuff   = ""
#                                             )
