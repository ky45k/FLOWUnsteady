...
run_name        = "ideal"      # Name of this simulation
save_path       = run_name
...
# Rotor geometry
rotor_file      = "ideal.csv"             # Rotor geometry
...
RPM               = 5500
# RPM             = 1250                      # RPM NACA0012
# J               = 0.2678                  # Hovering
# J               = 0.485437                    # Advance ratio Vinf/(nD)
J               = 0.000001
...
Vinf(X, t)      = magVinf*[sin(AOA*pi/180), cos(AOA*pi/180), 0]  # (m/s) freestream velocity vector
...
max_particles   = ((2*n+1)*B)*nsteps*p_per_step + 1000000 # Maximum number of particles
...
sigmafactor_vpmonvlm= 6                     # Shrink particles by this factor when
                                            #  calculating VPM-on-VLM/Rotor induced velocities
...
# hubtiploss_correction = ((0.44,5,0.1,0.05),(2, 1, 0.25, 0.05))
hubtiploss_correction = ((0.65, 5, 0.1, 0.05),(2, 1, 1, 0.05))
# hubtiploss_correction = ((1, 5, 0.1, 0.05),(2, 1, 0.25, 0.05))
...
vpm_viscous   = vpm.CoreSpreading(1.4776e-5, 0.006, vpm.zeta_fmm; beta=1.8, itmax=20, tol=1e-2)
...
vpm_fmm = vpm.FMM(; p=4,
                   ...
                   min_ncrit=10)  # FMM은 Julia 스레드 자동 사용
...
function polaRe(sim, pfield, T, DT; vprintln=uns._dummy)
    ...
    function select_airfoil(roR::Float64)
        ...
        # elseif
        if haskey(polar_data_map, "NACA0012") && roR >= 0.2
            return "NACA0012"  # 팁 쪽
        else
            # 사용 가능한 첫 번째 에어포일 사용
    ...
    for i in 1:size(rotor.airfoils, 1)
        ...
        if size(data_CL) != (length(aoa_vec), length(Re_vec))
            vprintln("Error: Data size mismatch for $airfoil_key")
            vprintln("  AOA vec length: $(length(aoa_vec)), Re vec length: $(length(Re_vec))")
            vprintln("  Data CL size: $(size(data_CL))")
            continue  # 이 섹션은 건너뛰고 다음으로
        end
...
# Restart simulation
restart_file = "/home/kweon/rotor_180/rotor_180_pfield.2880"
...
# Generate sectional coefficients monitor
monitor_sectional = uns.generate_monitor_sectional_coeffs(rotors, rho, nsteps;
                                            show_final_only=true,  # 이전 값 지우기
                                            save_path=save_path,
                                            run_name=run_name,
                                            figname="sectional coefficients monitor")

# Generate airfoil pressure monitor at specific r/R stations
monitor_airfoil_pressure = uns.generate_monitor_airfoil_pressure(rotors, rho, nsteps;
                                            r_R_stations=[0.68, 0.8, 0.96],  # 원하는 r/R 위치들
                                            save_path=save_path,
                                            run_name=run_name,
                                            figname="airfoil pressure monitor")

# Concatenate monitors
monitors = uns.concatenate(monitor_rotor, monitor_enstrophy, monitor_Cd, monitor_sectional, monitor_airfoil_pressure)
...
runtime_function = uns.concatenate(monitors, wake_treatment_supress, polaRe)#, wake_treatment)
# Run simulation
...
println("\n" * "="^70)
println("          시뮬레이션 완료 - 성능 요약")
println("="^70)

println("\n[병렬 처리 구성]")
println("  • Julia 스레드 수: $(Threads.nthreads())")
if Threads.nthreads() > 1
    println("  • 병렬 처리: 활성화 ✓")
    println("  • 예상 속도 향상: 약 $(Threads.nthreads())배")
else
    println("  • 병렬 처리: 비활성 ⚠")
end
