...
"""
function remove_particles_cylinder(Pstart::Vector{Float64}, Pend::Vector{Float64}, R::Real, step::Int)
    ...
    function wake_treatment(sim, PFIELD, T, DT, args...; optargs...)
        if sim.nt % step == 0
            ...
            for i in vpm.get_np(PFIELD):-1:1
                P = vpm.get_particle(PFIELD, i)
                x = P.X .- Pstart           # 입자 위치를 원기둥 기준으로 이동

                h = dot(x, d_unit)          # 축 방향 투영
                r_squared = norm(x .- h .* d_unit)^2  # 수직 거리의 제곱

                if (h < 0 || h > L || r_squared > R^2)
                    vpm.remove_particle(PFIELD, i)
                end
            end
