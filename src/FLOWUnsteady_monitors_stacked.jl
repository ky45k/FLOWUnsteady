...
function generate_monitor_rotors( rotors::Array{vlm.Rotor, 1},
    ...
    function extra_runtime_function(sim::Simulation{V, M, R},
        ...
        if fcalls==0
            ...
            if save_path!=nothing
                f = open(fname, "w")
                print(f, "ref age (deg),T,DT,RPM,CT,CQ,eta")
                print(f, "\n")
                close(f)
            end
        ...
        if PFIELD.nt % nsteps_plot == 0 && disp_conv
            CTs, CQs, RPMs = Float64[], Float64[], Float64[]
            ...
            CQ_total = sum(CQs)
            RPM_avg = mean(RPMs)
            eta_total = J_ref*CT_total / (2*pi*CQ_total)
        
            # Plot
            axs[4].plot([t_scaled], [CT_total], "o-", alpha=alpha, color=clr)
            axs[5].plot([t_scaled], [CQ_total], "o-", alpha=alpha, color=clr)
            axs[6].plot([t_scaled], [eta_total], "o-", alpha=alpha, color=clr)

            # Save
            if save_path != nothing
                print(f, ",", RPM_avg, ",", CT_total, ",", CQ_total, ",", eta_total)
            end
        end
