...
"""
    add_total_ct_to_convergence(filepath::String; num_rotors::Int=2, output_filepath::String="")

후처리 함수로, 기존에 생성된 convergence.csv 파일을 읽어서 모든 로터의 CT 값을 합한 
총 CT 값을 계산하고 이를 포함한 새 파일을 생성합니다.

* `filepath`: 원본 convergence.csv 파일 경로
* `num_rotors`: 로터의 개수
* `output_filepath`: 출력 파일 경로 (지정하지 않으면 자동 생성)

# 사용 예시:
```julia
add_total_ct_to_convergence("경로/rotor_convergence.csv", num_rotors=2)
```
"""
function add_total_ct_to_convergence(filepath::String; 
                                    num_rotors::Int=2, 
                                    output_filepath::String="")
    
    # 출력 파일 경로가 지정되지 않은 경우 기본값 설정
    if isempty(output_filepath)
        dir, fname = splitdir(filepath)
        base, ext = splitext(fname)
        output_filepath = joinpath(dir, base * "_with_total" * ext)
    end
    ...
    if length(lines) <= 1
        @warn "데이터가 부족합니다: $filepath"
        return
    end
    
    # 새 파일 생성
    open(output_filepath, "w") do out_file
        # 헤더 라인 처리
        header = lines[1]
        println(out_file, header * ",CT_total")
        ...
        for i in 2:length(lines)
            ...
            # CT 값 찾기
            ct_values = Float64[]
            for r in 1:num_rotors
                # 원본 파일 형식에 맞게 인덱스 계산
                # 형식: ref age, T, DT, RPM_1, CT_1, CQ_1, eta_1, RPM_2, CT_2, ...
                ct_index = 3 + 4*(r-1) + 2  # 첫 3개 컬럼 + 4*(로터인덱스-1) + 2(CT 위치)
                
                if ct_index <= length(parts)
                    try
                        push!(ct_values, parse(Float64, parts[ct_index]))
                    catch e
                        @warn "CT 값 파싱 실패 (줄 $i, 로터 $r): $(parts[ct_index])"
                    end
                end
            end
            
            # 총 CT 계산
            total_ct = sum(ct_values)
            
            # 기존 데이터에 total CT 추가
            println(out_file, line * ",$(total_ct)")
        end
    end
    
    println("Total CT 추가 완료: $output_filepath")
